...
## NAT Resilience and Cost

```text
Estimate = hourly charge x hours
         + data-processing charge x GB
         + applicable transfer and public IPv4 charges
```

| Design                             | Result                                            |
| ---------------------------------- | ------------------------------------------------- |
| NAT-A for Private-A only           | Cost-safe learner test; Private-B has no NAT path |
| NAT-A for both private subnets     | Single-AZ dependency and possible cross-AZ cost   |
| NAT-A + NAT-B with same-AZ routing | Classic resilient design; two hourly charges      |

Do not memorize a Region's price. Check current AWS pricing before production
design or longer practice.
...
Flow Logs record network metadata for a VPC, subnet, or ENI. Destinations can
include CloudWatch Logs, S3, or Data Firehose. They do not capture packet
payloads.

Useful fields:

- Interface, source, and destination addresses
...
An `ACCEPT` record proves the network controls accepted the flow; it does not
prove the application listener or process worked.

## Key Decisions

1. Private IPv4 internet egress: private route table -> public NAT -> IGW.
2. Private S3 path: Gateway Endpoint.
3. Private AWS API ENI: Interface Endpoint and PrivateLink.
4. Workload firewall: Security Group.
5. Subnet-level explicit deny: NACL.
